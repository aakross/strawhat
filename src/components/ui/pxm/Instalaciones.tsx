...
export default function Instalaciones() {
  ...
  const { services, amenities, recommendations } = useServices();
    ...
    <>
      <section className="relative w-full min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 py-12 md:py-20 lg:py-24">
          ...
          <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 lg:gap-16 max-w-6xl mx-auto">
            <ServiceColumn items={services} />
            <ServiceColumn items={amenities} />
            <ServiceColumn items={recommendations} />
          </div>

          {/* Versión móvil con una columna - SOLO EN MÓVIL */}
          <div className="md:hidden">
            <div className="grid grid-cols-1 gap-4 max-w-2xl mx-auto">
              {[...services, ...amenities, ...recommendations].map(
                (item, index) => (
                  <div
                    key={index}
